#!/usr/bin/env python3
"""
Delta Exchange India - BTCUSD Perpetual Futures SuperTrend Bot
Single-file production trading bot running on 5m candles.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("delta_futures_bot.log", mode="a")
    ]
)
...
SYMBOL = "BTCUSD"
PRODUCT_ID = 27           # Delta Exchange product ID for BTCUSD
TICK_SIZE = 0.5           # BTCUSD tick size
...
TOTAL_LOTS = 30
TP_SPLIT_LOTS = [10, 10, 10]
ENTRY_OFFSET = 50.0       # Points offset from signal close for limit entry
TP_OFFSETS = [300.0, 600.0, 900.0]

# ------------------------------------------------------------------------------
# Utility Functions
# ------------------------------------------------------------------------------
def round_to_tick(price: float, tick_size: float = TICK_SIZE) -> float:
    """Rounds price to the nearest tick size precision."""
...
class DeltaAPIClient:
    """REST API Client for Delta Exchange India Production API."""
...
class DeltaFuturesBot:
    """Main trading bot controller handling state management, execution, and restart recovery."""
    ...
    def cancel_all_orders(self) -> None:
        """Cancels all active open and pending orders."""
    ...
    def close_position_and_wait(self) -> bool:
        """
        Closes existing position via market order and waits until position size is confirmed zero.
        """
        ...
        # Wait and verify position closure
    ...
    def sync_take_profits(self) -> None:
        ...
        Ensures exact TP coverage without duplicate orders.
        """
        pos_size, entry_price = self.get_position_size_and_entry()
        if pos_size == 0 or entry_price <= 0:
            # If no open position, cancel orphan reduce_only orders
            orders = self.api.get_active_orders(PRODUCT_ID)
            for order in orders:
        ...
        # Determine target TP prices based on actual position entry price
        target_tp_prices = []
    ...
    def process_signals(self) -> None:
        """Main signal check and trade execution flow."""
        ...
        # Ensure we evaluate only COMPLETED candles (exclude current forming candle)
        current_time = int(time.time())
        latest_candle_time = candles[-1]["time"]
        
        # If the last candle in list is still open, use previous completed candle
        ...
        last_completed_time = completed_candles[-1]["time"]
        if last_completed_time <= self.last_processed_candle_time:
            # Already evaluated this candle
            return
        ...
        self.last_processed_candle_time = last_completed_time
        logger.info(
            f"Candle Completed [{datetime.fromtimestamp(last_completed_time, tz=timezone.utc).strftime('%H:%M UTC')}]: "
        ...
        if prev_trend == -1 and curr_trend == 1:
            logger.info(f"BUY Signal Triggered! Signal Close: {signal_close}")
            target_limit_price = round_to_tick(signal_close - ENTRY_OFFSET)

            if pos_size < 0:
                logger.info("Reversal required: Short position currently open. Closing position first...")
                if not self.close_position_and_wait():
                    logger.error("Skipping new BUY limit order because old position failed to close cleanly.")
                    return
            else:
                self.cancel_all_orders()
        ...
        # SELL Signal: Bullish -> Bearish
            ...
            logger.info(f"SELL Signal Triggered! Signal Close: {signal_close}")
            target_limit_price = round_to_tick(signal_close + ENTRY_OFFSET)

            if pos_size > 0:
                logger.info("Reversal required: Long position currently open. Closing position first...")
                if not self.close_position_and_wait():
                    logger.error("Skipping new SELL limit order because old position failed to close cleanly.")
                    return
            else:
                self.cancel_all_orders()
    ...
    def run(self) -> None:
        """Starts continuous trading bot execution loop with restart recovery."""
        logger.info("==================================================")
        logger.info("Starting Delta Exchange India Futures Trading Bot")
        ...
        # Restart Recovery Sync
        logger.info("Performing restart recovery and initial state synchronization...")
        try:
            self.sync_take_profits()
        except Exception as e:
            logger.error(f"Error during restart recovery sync: {e}")

        # Main Loop
        while True:
            try:
                # 1. Synchronize TP orders with active position
                self.sync_take_profits()

                # 2. Check for newly completed candles and process signals
